import { emitFile, getDoc, getFormat, getNamespaceFullName, isArrayModelType, isRecordModelType, isStdNamespace, isTemplateDeclaration, navigateProgram, resolvePath, } from "@typespec/compiler";
...
function renderFile(ns, usings, block) {
    const lines = [];
...
function renderModel(program, model, options) {
    const lines = [];
    const doc = getDoc(program, model);
    if (doc)
        lines.push(...renderDocComment(doc));
    const className = pascalCase(model.name);
    const interfaceName = `I${className}`;
    const bases = [];
    if (model.baseModel)
        bases.push(typeReference(model.baseModel));
...
function renderInterface(program, model, options) {
    const lines = [];
    const doc = getDoc(program, model);
    if (doc)
        lines.push(...renderDocComment(doc));
...
function renderClassProperty(program, prop, options) {
    const lines = [];
...
function renderInterfaceProperty(program, prop, options) {
    const lines = [];
    const doc = getDoc(program, prop);
    if (doc)
        lines.push(...renderDocComment(doc));
